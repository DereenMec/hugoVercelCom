...
在上面的例子中，我们使用`if(input)`来判断文件读取是否成功，可以这样做的原因是如果打开文件失败，流`input `的failbit位就会被置位，条件也自然变为假。

#### 自动构造和析构

加入有这样一个程序，他的main函数接受一个要处理的文件列表。这种程序可能会有如下的循环：
...
💡这里需要注意的是我们并没有手动执行`input.close()`，这是因为局部变量input在被销毁时，close会被自动调用。
...
| 操作            | 含义                                                         |
| --------------- | ------------------------------------------------------------ |
| stream strm;    | 创建一个未绑定的stringstream对象                             |
| stream strm(s); | strm是一个sstream对象，保存string s的一个拷贝，此构造函数是explicit的 |
